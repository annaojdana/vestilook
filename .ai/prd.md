...
| **US-005** | Generowanie wizualizacji (sukces) | Jako użytkownik, chcę przesłać zdjęcie ubrania i uruchomić proces VTON, aby otrzymać fotorealistyczny wynik. | 1. Zdjęcie ubrania przechodzi walidację (US-006).<br>2. Użytkownik ponownie klika checkbox zgody przed wywołaniem API (FR-007).<br>3. Po kliknięciu "Generuj", wyświetlany jest szczegółowy stan ładowania.<br>4. System zwraca wygenerowany obraz VTON, który jest tymczasowo przechowywany w Supabase Storage. |
